...
@testset "mapreduce" begin
    @testset "corner cases with small arrays" begin
        ...
        @testset for (op, init) in ((+, 0), (*, 1))
            @test reduce(op, Fill(2,0)) == reduce(op, fill(2,0))
            @test reduce(op, Fill(2,0); init) == reduce(op, fill(2,0); init)
            @test reduce(op, Fill(2,0), dims=1) == reduce(op, fill(2,0), dims=1)
            @test reduce(op, Fill(2,0); init, dims=1) == reduce(op, fill(2,0); init, dims=1)
        end
        @testset for (op, init) in ((&, true), (|, false))
            @test reduce(op, Fill(true,0)) == reduce(op, fill(true,0))
            @test reduce(op, Fill(true,0); init) == reduce(op, fill(true,0); init)
            @test reduce(op, Fill(true,0), dims=1) == reduce(op, fill(true,0), dims=1)
            @test reduce(op, Fill(true,0); init, dims=1) == reduce(op, fill(true,0); init, dims=1)
        end
        @test reduce(vcat, Fill(2,0), init=1) == reduce(vcat, fill(2,0), init=1)
        @test reduce(vcat, Fill(2,1), init=1) == reduce(vcat, fill(2,1), init=1)
        @testset for (op, init) in ((max, 0), (+, 0), (*, 1))
            @test reduce(op, Fill(2,0), dims=2) == reduce(op, fill(2,0), dims=2)
            @test reduce(op, Fill(2,0,1), dims=2) == reduce(op, fill(2,0,1), dims=2)
            @test reduce(op, Fill(2,0); init, dims=2) == reduce(op, fill(2,0); init, dims=2)
            @test reduce(op, Fill(2,0,1); init, dims=2) == reduce(op, fill(2,0,1); init, dims=2)

            @test reduce(op, Fill(2,1)) == reduce(op, fill(2,1))
            @test reduce(op, Fill(2,1), dims=1) == reduce(op, fill(2,1), dims=1)
            @test reduce(op, Fill(2,1), dims=2) == reduce(op, fill(2,1), dims=2)
            @test reduce(op, Fill(2,1,1), dims=1) == reduce(op, fill(2,1,1), dims=1)
            @test reduce(op, Fill(2,1,1), dims=2) == reduce(op, fill(2,1,1), dims=2)
            @test reduce(op, Fill(2,1); init, dims=1) == reduce(op, fill(2,1); init, dims=1)
            @test reduce(op, Fill(2,1); init, dims=2) == reduce(op, fill(2,1); init, dims=2)
            @test reduce(op, Fill(2,1,1); init, dims=1) == reduce(op, fill(2,1,1); init, dims=1)
            @test reduce(op, Fill(2,1,1); init, dims=2) == reduce(op, fill(2,1,1); init, dims=2)

            @test reduce(op, Fill(2,2)) == reduce(op, fill(2,2))
            @test reduce(op, Fill(2,2), dims=1) == reduce(op, fill(2,2), dims=1)
            @test reduce(op, Fill(2,2), dims=2) == reduce(op, fill(2,2), dims=2)
            @test reduce(op, Fill(2,2,2), dims=1) == reduce(op, fill(2,2,2), dims=1)
            @test reduce(op, Fill(2,2,2), dims=2) == reduce(op, fill(2,2,2), dims=2)
            @test reduce(op, Fill(2,2); init, dims=1) == reduce(op, fill(2,2); init, dims=1)
            @test reduce(op, Fill(2,2); init, dims=2) == reduce(op, fill(2,2); init, dims=2)
            @test reduce(op, Fill(2,2,2); init, dims=1) == reduce(op, fill(2,2,2); init, dims=1)
            @test reduce(op, Fill(2,2,2); init, dims=2) == reduce(op, fill(2,2,2); init, dims=2)
        end
    end
