...
@testset "Sub-arrays" begin
    ...
    @test @inferred(A[1:3]) ≡ Fill(3.0,3)
    @test @inferred(A[1:3,1:1]) ≡ Fill(3.0,3,1)
    @test_broken A[1:3,2] ≡ Zeros{Int}(3)
    @test_throws BoundsError A[1:26]
    @test @inferred(A[[true, false, true, false, false]]) ≡ Fill(3.0, 2)
    A = Fill(3.0, 2, 2)
    @test @inferred(A[[true true; true false]]) ≡ Fill(3.0, 3)
    @test_throws DimensionMismatch A[[true, false]]
    @test @inferred(B[1,2,3,2:4,2:5]) ≡ Fill(3.0,3,4)
    @test @inferred(B[:,1,:,2,:]) ≡ Fill(3.0,5,5,5)
    @test @inferred(B[[true,false,true,false,true],CartesianIndex(1,2),4,3:-1:2]) ≡ Fill(3.0,3,2)
    @test @inferred(B[CartesianIndices((3:4,1:1)),CartesianIndex(1,2),:]) ≡ Fill(3.0,2,1,5)

    A = Ones{Int}(5,5)
    B = Ones{Int}(5,5,5,5,5)
    @test @inferred(A[1:3]) ≡ Ones{Int}(3)
    @test @inferred(A[1:3,1:2]) ≡ Ones{Int}(3,2)
    @test @inferred(A[1:3,2]) ≡ Ones{Int}(3)
    @test_throws BoundsError A[1:26]
    A = Ones{Int}(2,2)
    @test A[[true false; true false]] ≡ Ones{Int}(2)
    @test A[[true, false, true, false]] ≡ Ones{Int}(2)
    @test_throws DimensionMismatch A[[true false false; true false false]]
    @test @inferred(B[1,2,3,2:4,2:5]) ≡ Ones{Int}(3,4)
    @test @inferred(B[:,1,:,2,:]) ≡ Ones{Int}(5,5,5)
    @test @inferred(B[[true,false,true,false,true],CartesianIndex(1,2),4,3:-1:2]) ≡ Ones{Int}(3,2)
    @test @inferred(B[CartesianIndices((3:4,1:1)),CartesianIndex(1,2),:]) ≡ Ones{Int}(2,1,5)

    A = Zeros{Int}(5,5)
    B = Zeros{Int}(5,5,5,5,5)
    @test @inferred(A[1:3]) ≡ Zeros{Int}(3)
    @test @inferred(A[1:3,1:2]) ≡ Zeros{Int}(3,2)
    @test @inferred(A[1:3,2]) ≡ Zeros{Int}(3)
    @test_throws BoundsError A[1:26]
    A = Zeros{Int}(2,2)
    @test @inferred(A[[true false; true false]]) ≡ Zeros{Int}(2)
    @test @inferred(A[[true, false, true, false]]) ≡ Zeros{Int}(2)
    @test_throws DimensionMismatch A[[true false false; true false false]]
    @test @inferred(B[1,2,3,2:4,2:5]) ≡ Zeros{Int}(3,4)
    @test @inferred(B[:,1,:,2,:]) ≡ Zeros{Int}(5,5,5)
    @test @inferred(B[[true,false,true,false,true],CartesianIndex(1,2),4,3:-1:2]) ≡ Zeros{Int}(3,2)
    @test @inferred(B[CartesianIndices((3:4,1:1)),CartesianIndex(1,2),:]) ≡ Zeros{Int}(2,1,5)
end
